...
# Local Worker dev (D1+R2 via --remote, talks to prod data)
...
Publishing is web-only (POST /api/submit). Retracting is web-only too —
self-service via `/me/articles/<uuid>` (cookie session) or `POST
/api/me/articles/<uuid>/retract` (token or cookie). The historical
`tools/unpublish.py` admin CLI was removed in Phase D once /me retract
shipped — every retract path now goes through the same D1 transition
that marks `retracted_at` and 410s the live URL.
...
- "Touch the feed/sitemap" → `worker/src/pages/feed.ts` is RSS 2.0
  (latest 30, newest first; cached 10 min). `worker/src/pages/sitemap.ts`
  is the XML sitemap (every live article + evergreen chrome pages;
  cached 1h). Both pull from `listRecentArticles()` and re-render
  per-request — no build step.
- "Turn on Cloudflare Web Analytics" → paste the token from
  *CF Dashboard → Web Analytics → paiink.com* into
  `worker/src/analytics.ts` (the `CF_ANALYTICS_TOKEN` constant) and
  redeploy. Empty string = no beacon emitted. CSP already whitelists
