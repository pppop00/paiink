...
**Do not** use `hokind`, `Hokind`, or the Andrew CMU email anywhere in this
repo. The user has corrected this twice and force-pushed `main` once to
scrub `hokind` from history. Treat this rule as load-bearing.
...
- **`paink-d19o.ipfs.4everland.app` is the production pointer.** Per-deploy
  subdomains (`paink-<hash>-pppop00.ipfs.4everland.app`) are immutable
  historical snapshots — never link to them from the site itself.
- **Force-push to main rewrites `paink-d19o`** the next time 4EVERLAND
  builds. Existing per-deploy CIDs are not affected. Don't force-push
  unless you genuinely want to scrub history (the user authorized once
  for the `hokind` purge; ask before doing it again).
