...
def build_sql(articles: list[dict], founder_created_at: int) -> str:
    lines: list[str] = []
    lines.append("-- migrate.sql — apply with `npx wrangler d1 execute paiink --remote --file tools/migrate.sql`")
    lines.append("-- Generated by tools/migrate_to_d1.py — do not hand-edit; rerun the generator.")
    lines.append("--")
    lines.append("-- Note: no BEGIN/COMMIT — D1 rejects explicit SQL transactions. wrangler")
    lines.append("-- d1 execute already applies the file as a single atomic batch via the D1 API.")
    lines.append("")
...
def build_shell(articles: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("# Use the wrangler version pinned in worker/node_modules. Bare `npx wrangler`")
    lines.append("# sometimes resolves to wrangler@4 which requires Node 22+; the local pin")
    lines.append("# (3.114.17) supports the Node version this repo was built against.")
    lines.append("WRANGLER=\"worker/node_modules/.bin/wrangler\"")
    lines.append("")
